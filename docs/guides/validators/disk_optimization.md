...
speed and transaction amount. For Uptick, we are talking about close to
...
I believe this was disabled by default on Uptick, but listing it in any
...
will risk database corruption if the `uptickd` is killed for any reason.
...
Below is the disk usage after two weeks of Uptick Arsia Mons testnet. The default
