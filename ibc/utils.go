...
import (
	...
	uptick "github.com/UptickNetwork/uptick/types"
...
func GetTransferSenderRecipient(packet channeltypes.Packet) (
	...
	// and change the bech32 human readable prefix (HRP) of the sender to `uptick`
	sender, err = uptick.GetUptickAddressFromBech32(data.Sender)
	if err != nil {
		return nil, nil, "", "", sdkerrors.Wrap(err, "invalid sender")
	}

	// validate the recipient bech32 address from the counterparty chain
	// and change the bech32 human readable prefix (HRP) of the recipient to `uptick`
	recipient, err = uptick.GetUptickAddressFromBech32(data.Receiver)
	if err != nil {
