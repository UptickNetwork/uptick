...
// consulting the wiring-time checker. A nil checker (a module-only setup with
// no contract side, such as the collection module's own unit tests) reports
// false, which preserves the unguarded behaviour those callers expect.
func (k Keeper) IsConvertedNFT(ctx sdk.Context, classID, nftID string) bool {
	if k.convertedNFTs == nil {
		return false
	}
	return k.convertedNFTs.IsConvertedNFT(ctx, classID, nftID)
}

// SetConvertedNFTChecker wires the cross-module burn guard.
//
// It must be called by the application once x/erc721 and x/cw721 exist -- they
// are constructed *from* this keeper (their NewKeeper takes it by value), so
// the dependency cannot be a constructor parameter. app/keepers/keepers.go
