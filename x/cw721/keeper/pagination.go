...
// scan-and-discard rows (potentially the entire store). Report these as
...
//
// This is deliberately the same contract x/collection/keeper/pagination.go
// implements for the denom/NFT queries. TokenPairs used to hand the request
// straight to query.Paginate, so the same client call was rejected on one
// module and silently scanned on the other. app/twin_pagination_parity_test.go
// fails if the three copies drift apart again.
func shapePageRequest(req *query.PageRequest) (*query.PageRequest, error) {
	...
	// full scan. Both are unbounded on a large pair table and were silently
