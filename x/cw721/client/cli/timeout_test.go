...
// TestTransferCW721Cmd_DefaultTimeouts is the regression guard for the G-15
// fix: with no timeout flags the CLI must fill in a fallback timestamp so the
// default invocation passes MsgTransferCW721.ValidateBasic instead of failing
// with "timeout height and timeout timestamp cannot both be zero".
func TestTransferCW721Cmd_DefaultTimeouts(t *testing.T) {
